# 📘 ANN Intermediate Project 

## 📘 Customer Churn Prediction using PyTorch

### Table of Contents

#### Chapter 1: [Introduction](#1-introduction)

#### Chapter 2: [Problem Statement](#2-problem-statement)

#### Chapter 3: [Understanding the Dataset](#3-understanding-the-dataset)

#### Chapter 4: [Exploratory Data Analysis (EDA)](#4-exploratory-data-analysis-eda)

#### Chapter 5: [Data Preprocessing](#5-data-preprocessing)

  - Chapter 5.1: [Handling Missing Values](#51-handling-missing-values)
  - Chapter 5.2: [Encoding Categorical Variables](#52-encoding-categorical-variables)
  - Chapter 5.3: [Feature Scaling](#53-feature-scaling)

#### Chapter 6: [Train-Test Split](#6-train-test-split)

#### Chapter 7: [Building the ANN Model](#7-building-the-ann-model)

  - Chapter 7.1: [Model Architecture](#71-model-architecture)
  - Chapter 7.2: [Activation Functions](#72-activation-functions)
  - Chapter 7.3: [Dropout Layer](#73-dropout-layer)

* Chapter 8: [Loss Function and Optimizer](#8-loss-function-and-optimizer)

* Chapter 9: [Training the Model](#9-training-the-model)

  * Chapter 9.1: [Training Loop](#91-training-loop)
  * Chapter 9.2: [Validation Step](#92-validation-step)

* Chapter 10: [Model Evaluation](#10-model-evaluation)

  * Chapter 10.1: [Accuracy](#101-accuracy)
  * Chapter 10.2: [Precision and Recall](#102-precision-and-recall)
  * Chapter 10.3: [F1 Score](#103-f1-score)

* Chapter 11: [Improving the Model](#11-improving-the-model)

  * Chapter 11.1: [Hyperparameter Tuning](#111-hyperparameter-tuning)
  * Chapter 11.2: [Trying Different Architectures](#112-trying-different-architectures)

* Chapter 12: [Final Results and Conclusion](#12-final-results-and-conclusion)

* Chapter 13: [Student Exercises](#13-student-exercises)

* Chapter 14: [Mini Projects](#14-mini-projects)

* Chapter 15: [References](#15-references)

---

If you want, I can next convert this into a **student-friendly version with very simple explanations under each chapter** (good for your teaching style).

